...
abstract class DataAccessor {
    ...
    protected function initialize($collection){
        ...
        $connexionString = 'mongodb://'.self::$connexionData['host'].':'.self::$connexionData['port'];
        $mongo = new Mongo($connexionString, array("persistent" => self::$connexionData['database']));
        $this->collection = $mongo->selectCollection(self::$connexionData['database'], $collection);
    }
